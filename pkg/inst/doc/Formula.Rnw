...
\code{formula = y ~ x}, \code{instruments = ~ z} can simply do

\begin{Code}
formula <- if(!is.null(instruments)) as.Formula(formula, instruments)
  else as.Formula(formula)
stopifnot(isTRUE(all.equal(length(formula), c(1, 2))))
\end{Code}
