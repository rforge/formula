...
\Abstract{ 
  ...
  with two or more sets of variable, e.g., regressors/intruments in instrumental
...
and all LHS variables can be extracted via
%
<<multi-response4>>=
...
in using \class{formula} objects is oftern to compute the associated
...
and potentially further variables like weights, offset, etc. A model frame
is simply a \class{data.frame} with additional attributes (including \class{terms})
but without a specific class. From this model frame several components can
...
Last not least, \fct{model.matrix} can compute ``design'' or ``regressor''
...
@
%
Based on this model frame the model matrices for each RHS part can be
easily computed, again setting the \code{rhs} argument:
%
<<model.matrix-method>>=
...
a second RHS part with the instruments. The function \fct{ivcoef} processes
these arguments in the following four steps: (1)~process the call,
(2)~set up the model frame (using the \class{Formula} method),
(3)~extract response and regressors,
(4)~call \fct{lm.fit} twice to compute the two-stage least squares coefficients.
%
<<ivcoef>>=
...
@
%
The resulting funtion can then be applied easily (albeit not very meaningfully)
