...
<<details1>>=
F3 <- Formula(y1 + y2 | y3 ~ x1 + I(x2^2) | 0 + log(x1) | x3 / x4)
F3
length(F3)
attr(F3, "lhs")
attr(F3, "rhs")
...
model.frame(terms(F3, lhs = 1, rhs = 0), data = mf3)
model.frame(terms(F3, lhs = 2, rhs = 0), data = mf3)
@
